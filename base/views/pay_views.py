...
class PaySuccessView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        # Get the latest Order object and change to confirm order
        order = Order.objects.filter(
            user=request.user).order_by('-created_at')[0]
        order.is_confirmed = True  # Confirm Order
        order.save()
...
class PayCancelView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        # Get the latest Order object
        order = Order.objects.filter(
            user=request.user).order_by('-created_at')[0]
 
        # Restore inventory and sales numbers to their original state.
        ...
        # If is_confirmed is False, then delete (temporary order deletion)
...
tax_rate = stripe.TaxRate.create(
    ...
    inclusive=False,  # Specify external tax (True for internal tax)
)
...
class PayWithStripe(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        # Check to see if customer profile is filled
        if not check_profile_filled(request.user.profile):
            messages.error(request, 'Please fill out your profile for delivery.')
        ...
        for item_pk, quantity in cart['items'].items():
            ...
            # Stock should be subtracted at this point, if order is cancelled, stock will be returned.
            # Add up the number of sold count.
            item.stock -= quantity
            item.sold_count += quantity
            item.save()
 
        # Create a provisional order (is_confirmed=Flase)
